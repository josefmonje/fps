...
.service('pointerlock', [
  function () {
    ...
    this.exit  = function () {
      return document.exitPointerLock || document.mozExitPointerLock || document.webkitExitPointerLock
    }
  }
])
.service('fullscreen', [
...
.controller('PersistentCtrl', ['$scope', '$document', 'moveCfgs', 'treeCfgs', 'pointerlock',
  function ($scope, $document, moveCfgs, treeCfgs, pointerlock) {
    ...
    $scope.start = function (element) {
      element.onclick = $scope.createBullet
      element.onkeydown = onKeyDown
      element.onkeyup = onKeyUp
      startControls(element)
    }

    var endControls = function (element) {
      element.exitPointerLock = pointerlock.exit()
      element.exitPointerLock()
    }

    $scope.end = function (element) {
      endControls(element)
    }
    ...
    $scope.activateControls = function (element) {
      var start = function () { $scope.start(element) }
      var end = function () { $scope.end(element) }
      element.onpointerlockchange = lockChange
      element.onmozpointerlockchange = lockChange
      element.onwebkitpointerlockchange = lockChange
      element.onclick = start
      element.onpointererror = end
      element.onmozpointererror = end
      element.onwebkitpointererror = end
    }
    ...
    $scope.createBullet = function () {
      ...
      startControls($document[0]) // so all clicks to screen regain pointer lock
    }
...
.controller('homeCtrl', ['$scope', '$document', 'pointerlock',
  function ($scope, $document, pointerlock) {
    $scope.end($document[0])
  }
])
.controller('roomCtrl', ['$routeParams', '$scope', 'pointerlock', '$document', '$controller',
  function ($routeParams, $scope, pointerlock, $document, $controller) {
    ...
    var module = angular.injector(['ng', 'rooms'])
    if (module.has(config)) { config = module.get(config) } // http.get this
      else config = module.get('defaultCfgs')

    if (pointerlock.available()) {
      $scope.$on('$routeChangeSuccess', $scope.activateControls($document[0]))
    }

    $scope.init(config)
    $scope.animate()
    try { $controller(controller, { $scope: $scope }) }
      catch (e) {}
  }
])
...
.controller('deepforestCtrl', ['$scope', 'deepforestCfgs',
  function ($scope, deepforestCfgs) {
    ...
    $scope.createBox(deepforestCfgs, 20)
  }
])
